...
void BlockTTMM(long n, long b, float**** A, float**** B, float**** C){
	...
	#define S1(i,j,k,l,i4,i5) C(i,j,l,i4) = 0
	#define S0(i,j,k,l,d,e) C(i,j,l,d) = (C(i,j,l,d))+((A(i,k,l,e))*(B(k,j,e,d)))
	{
		//Domain
		//{i,j,k,l,i4,i5|i5==0 && k==0 && n>=1 && b>=1 && i>=0 && j>=i && n>=j+1 && l>=0 && b>=l+1 && i4>=0 && b>=i4+1}
		//{i,j,k,l,d,e|i>=0 && k>=i && n>=k+1 && l>=0 && b>=l+1 && e>=0 && b>=e+1 && n>=1 && b>=1 && k>=0 && j>=k && n>=j+1 && b>=d+1 && d>=0 && j>=i}
		 	 	...
		 	 	for(c3=c1;c3 <= c2;c3+=1)
		 	 	 {
                    ...
                    cblas_sgemm(CblasRowMajor, //storage format: row major
                                ...
                                1, //multiply A by 1
                                *(A[c1][c3]), //A (assumes flat layout). In our case, this is the block at A[i][d]
                                b, //leading dimension of A
                                *(B[c3][c2]), //B (assumes flat layout). In our case, this is the block at B[d][j]
                                b, //leading dimension of B
                                1, //multiply C by this before adding AB
                                *(C[c1][c2]), //C (assumes flat layout). In our case, this is the block at C[i][j]
